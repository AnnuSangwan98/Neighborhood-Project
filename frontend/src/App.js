  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [neighborhoodResponse, crimeResponse] = await Promise.all([
        fetch(`http://localhost:5002/api/neighborhood-data?latitude=${searchParams.latitude}&longitude=${searchParams.longitude}&radius=${searchParams.radius}`),
        fetch(`http://localhost:5002/api/crime-data?city=${encodeURIComponent(searchParams.city)}`)
      ]);
